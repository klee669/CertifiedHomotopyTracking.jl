...
function intrinsic_radius(x)
    return min(BigFloat(1), ALPHA * x / 2)
end
...
function intrinsic_length(m)
    ...
    if endpoint <= breakpoint
        return 2 * log(endpoint) / ALPHA
    end
    return 2 * log(breakpoint) / ALPHA + endpoint - breakpoint
end
...
function main()
    setprecision(BigFloat, 256) do
        ...
        m_values = full ? [10^k for k in 1:8] : [10, 100]
        ...
        output_path = joinpath(@__DIR__, "results", "generated", "modified_univariate.csv")
        mkpath(dirname(output_path))
        write_results(output_path, results)
        println("\nWrote $output_path")

        theorem_constant = (1 + TAU) * (LAMBDA + TAU) / (TAU - RHO)
        comparison_path = joinpath(@__DIR__, "results", "generated", "radius_comparison.csv")
