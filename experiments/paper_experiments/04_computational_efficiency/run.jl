...
const RESULT_DIR = joinpath(@__DIR__, "results", "generated")
...
function run_all_tables_3rd_order(;
    ...
    for filename in ("table_5_1_order3_paths.partial.csv", "table_5_3_order3_paths.partial.csv")
...
if abspath(PROGRAM_FILE) == @__FILE__
    full = get(ENV, "CHT_FULL", get(ENV, "CHT_FULL_TABLES", "0")) == "1"
    max_paths_env = get(ENV, "CHT_MAX_PATHS", "")
    max_paths = isempty(max_paths_env) ? (full ? nothing : 1) : parse(Int, max_paths_env)
    start_path = parse(Int, get(ENV, "CHT_START_PATH", "1"))
    only_order3 = get(ENV, "CHT_ONLY_ORDER3", "0") == "1"
    methods = only_order3 ? [:order3_apriori] : ALL_5_3_METHODS
